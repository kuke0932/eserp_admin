...
<script>
  import { upload, remove, uploadLarge, removeLarge } from '@/api/file'
    ...
    computed: {
      dataTips: function() {
        if (this.tips) {
          ...
          return '温馨提示：最多传' + this.limitExceed + '个文件哦！！！'
        }
      }
    },
    methods: {
      ...
      fileChange(el) {
        ...
        let uploadMethod
        if (this.isLarge) {
          uploadMethod = uploadLarge
        } else {
          uploadMethod = upload
        }
        uploadMethod(el.target.files[0]).then(resp => {
      ...
      fileDel(index) {
        let removeMethod
        if (this.isLarge) {
          removeMethod = removeLarge
        } else {
          removeMethod = remove
        }
        removeMethod(this.dataImgList[index].attachmentId).then(resp => {
