<template>
  <div class="container">
    <el-form ref="searchForm" class="searchForm" :inline="true" :model="searchForm">
      <el-row :gutter="20">
        ...
        <el-form-item label="物料类型" prop="materialType" v-if="areaType === 1">
          <el-select v-model="searchForm.materialType" placeholder="物料类型" filterable>
    ...
    <el-dialog
      ...
      <el-form ref="addForm" :rules="rules" :model="addForm" label-width="80px" v-loading="addFormLoading" :element-loading-text="loadingText">
        ...
        <el-form-item label="物料编码" prop="code">
          <el-input clearable v-model="addForm.code" style="width: 93%; float: left;" :disabled="true">
            <el-button slot="append" code="add" @click="seeMaterialCode">+</el-button>
          </el-input>
        </el-form-item>
    ...
    <el-dialog
      title="请输入二维码数量"
      :visible.sync="qrCodeAmountVisible"
      width="40%"
      :before-close="handleClose">
      <el-form ref="qrCodeForm" :rules="rules" :model="qrCodeForm" label-width="80px"
               v-loading="qrCpdeFormLoading"
               :element-loading-text="loadingText">
        <el-form-item label-width="0px" label="" prop="amount">
          <el-input clearable type="textarea" v-model="qrCodeForm.amount" autosize></el-input>
        </el-form-item>

        <el-form-item class="btn_box">
          <el-button type="primary" :loading="submitLoading" @click="printQrCode">打印</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  ...
  export default {
      ...
      return {
        ...
        rules: {
          ...
          amount: [
            ...
            { pattern: /^[0-9]$/, message: '请输入數字', trigger: 'blur' }
          ]
        }
      }
    },
    ...
    methods: {
      ...
      goPrintQrCode(list) {
        ...
        this.qrCodeAmountVisible = true
      },
      /* 打印二維碼 */
      printQrCode() {
        if (this.qrCodeForm.amount === null || this.qrCodeForm.amount === '' || this.qrCodeForm.amount <= 0) {
          this.$message({
            type: 'warning',
            message: '请输入打印数量'
          })
        ...
        printQrCode(rows.join(','), this.qrCodeForm.amount).then(res => {
        ...
        this.qrCodeAmountVisible = false
        this.qrCodeForm.amount = null
        this.qrCpdeFormLoading = false
      }
    }
  }
</script>
